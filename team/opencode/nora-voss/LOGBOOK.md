...
## Shift 6 — T-33: per-material luminance→glyph ramps (art-directed no-fill)
...
## Shift 5 — T-02: Golden-frame regression harness
- **Scope**: new `tools/golden_capture.py`, `tests/test_golden_frames.py`, `tests/golden/**` (TASKBOARD T-02). No engine files touched — zero overlap with live tickets.
- **Harness**: renders a fixed 10-scenario matrix (day/night × clear/rain/fog × interior/driving/street-life × blocks/ascii-color/ascii-mono × 80×32/160×50) through the real Raycaster into sha256 framebuffer digests; manifest also stores zlib+base64 cell data so failures print **which cells changed** (`(x,y): 'old'→'new'` + rows-most-affected), not just "not equal". Verified the failure path by tampering one cell → exact pinpoint, exit 1.
- **Determinism contract** (documented in-module): seeded procedural city (seed 5); global RNG seeded around the ENTIRE scenario build and restored after; time-of-day frozen at construction; all simulation stepping fixed-dt; digests over a canonical glyph/fg/bg serialization (stable across processes/Python versions).
- **Finding worth recording**: entity modules consume the module-level `random` stream with no seeding anywhere in `src/` — confirms the T-08 premise by measurement. First harness draft seeded only at render time and driving/street-life frames drifted run-to-run; wrapping the full build fixed it. Also confirmed rain/fog particle state does not visibly reach the raycaster framebuffer (those cells were stable even unseeded) — weather visuals enter via density/wetness scalars.
- **Results**: 221/221 tests green (+6: matrix coverage, pixel lock w/ readable diff, blob↔digest integrity, in-process determinism ×2); golden suite costs 0.53 s standalone; full capture regenerates in ~0.9 s; byte-stable across 3 independent processes. T-04/T-26/T-29/T-36 unblock once this PR merges.
- **Hand-off note forwarded** (from Priya's inbox memo): `_render_volumetric` samples `art_fg[tex_y][tex_x]` guarded only by char-grid bounds — if `VolumetricSprite` frames can be ragged like `Sprite` was, that path deserves Priya's constructor-level normalization. Memo left in Marcus's inbox.
- **Next**: stand by for golden-regen requests from Phase 1 tickets; T-05/T-06/T-07 perf work once T-04 lands.
