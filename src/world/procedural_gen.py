...
class ProceduralCityGenerator:
    ...
    def generate(
        ...
        ns_lines = self._plan_axis(width, sx, rng)
        ew_lines = self._plan_axis(height, sy, rng)
        ...
        # Park planning may drop collectors/locals inside the green.
        ...
        props: List[Sprite] = []
        landmarks: List[Landmark] = []
        pier_ys = self._carve_harbor(
            width, height, ns_lines, ew_lines, walls, floors, districts,
            water_x0, sx, sy, rng
        )
        if has_marina:
            self._add_mooring_props(water_x0, pier_ys, walls, floors, props)
    ...
    def _plan_axis(self, dim: int, scale: float, rng: random.Random) -> List[_RoadLine]:
        ...
        for i in range(len(arterial_centers) - 1):
            a, b = arterial_centers[i], arterial_centers[i + 1]
            ...
            if free < col_w + 2 * max(4, round(6 * scale)):
                continue
            if rng.random() < 0.62:
                self._add_collectors(a, b, col_w, scale, rng, lines)
            else:
                self._maybe_add_local(corridor_lo, corridor_hi, loc_w, scale, rng, lines)
    ...
    def _plan_park_rect(self, width, height, sx, sy, rng,
        ...
        if len(ns_arts) >= 2 and len(ew_arts) >= 2:
            xi = min(range(len(ns_arts)),
                     key=lambda k: abs(ns_arts[k].center - (width / 2 - 24)))
            yi = min(range(len(ew_arts)),
    ...
    def _carve_harbor(self, width, height, ns_lines, ew_lines, walls, floors, districts,
        ...
        Returns the pier centre-row coordinates."""
    ...
    def _add_mooring_props(self, water_x0, pier_ys, walls, floors, props):
